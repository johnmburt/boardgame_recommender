...
def recommender_tab_advanced(recommender, allgames, genres, mechanics):
...
#         print(ctl_game_min_rating.value)
        genre_selections = [
            ctl_genre_selection1.labels[i] for i in ctl_genre_selection1.active]
        genre_selections += [
            ctl_genre_selection2.labels[i] for i in ctl_genre_selection2.active]
#         print(genre_selections)
...
#         src.data.update(new_src.data)
    ...
    def make_div_list(textlist, max_lines, fmt_str="""%s""", render_as_text=True):
        ...
        for i in range(max_lines):
            if len(textlist) > i:
                divs.append(Div(text=fmt_str%(textlist[i]), 
                                render_as_text=render_as_text,
#                                 margin=[1,1,1]
                               ))
            else:
                divs.append(Div(text=fmt_str%(' '), 
                                render_as_text=render_as_text,
#                                 margin=[1,1,1]
                               ))
        return divs
...
#     update_liked_list(list(games_sel['name'].sample(max_liked)))
    ...
    ctl_game_min_rating = Slider(start = 1, end = 10, value = 1,
        step = .1, title = 'Minimun average rating', 
                                 width_policy='min')
    ctl_game_min_rating.on_change('value', update_filters)

    # game genre checkbox group
    genre_list = ['Any category'] + list(genres['tag'].values)
    ctl_genre_selection1 = CheckboxGroup(
        labels=genre_list[:int(num_check_options/2)], 
        width_policy='min', active = [0])
    ctl_genre_selection1.on_change('active', update_filters)
    ctl_genre_selection2 = CheckboxGroup(
        labels=genre_list[int(num_check_options/2):num_check_options], 
        width_policy='min')
    ctl_genre_selection2.on_change('active', update_filters)
...
#     p = style(p)
    ...
    filter_controls = WidgetBox(
        ...
        row(
            column(Div(text="""<h3>Game Categories:</h3>"""),
                row(ctl_genre_selection1, ctl_genre_selection2)),
            Spacer(min_width=50),
    ...
    # Create a row layout
    layout = column(row(pref_controls, results_controls), filter_controls)
